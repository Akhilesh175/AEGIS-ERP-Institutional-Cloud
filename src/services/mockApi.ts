...
export const mockApi = {
  ...
  async login(email: string, password_hash: string): Promise<AuthSession> {
    ...
    if (user.role === 'STUDENT') {
      ...
      if (stRow) {
        const { data: fullSt } = await supabaseAdmin
          .from('students')
          .select(`
            *,
        ...
        if (fullSt) {
          const profileJoin = Array.isArray(fullSt.student_profiles)
            ? fullSt.student_profiles[0]
            : fullSt.student_profiles;
          const profilePhotoUrl: string =
            profileJoin?.student_id === fullSt.id && profileJoin?.school_id === fullSt.school_id
              ? (profileJoin?.photo_url || '')
              : '';

          const academicPhoto = fullSt.registration_photo_url || profilePhotoUrl || '';
    ...
    try {
      ...
      if (dbStudents) {
        dbStudents.forEach((r: any) => {
          const profileJoin = Array.isArray(r.student_profiles)
            ? r.student_profiles[0]
            : r.student_profiles;
          const profilePhotoUrl: string =
            profileJoin?.student_id === r.id && profileJoin?.school_id === r.school_id
              ? (profileJoin?.photo_url || '')
              : '';

          const academicPhoto = r.registration_photo_url || profilePhotoUrl || '';
  ...
  async adminGetStudents(): Promise<(Student & { userDetails: User; className: string })[]> {
    ...
    // registration_photo_url is ALWAYS fetched unconditionally (primary academic photo column).
    // profile_photo_url on users is conditional — may not exist on older DB schemas.
    const profilePhotoField = hasProfilePhotoUrlColumn ? ', profile_photo_url' : '';
    const userFields = `id, email, first_name, last_name, phone, avatar_url${profilePhotoField}, role, school_id, is_active, created_at`;
    const queryFields = `
      id, user_id, school_id, class_id, section_id, academic_session_id, admission_number, roll_number, date_of_birth, gender, created_at,
      registration_photo_url,
    ...
    let { data: studentRows, error } = await supabase
      .from('students')
      .select(queryFields)
      .eq('school_id', schoolId);

    // If query fails (e.g. registration_photo_url column not yet migrated), retry without it
    if (error) {
      console.warn('[adminGetStudents] Primary query failed, retrying without registration_photo_url:', error.message);
      const fallbackFields = `
        ...
        student_profiles(photo_url, student_id, school_id)
      `;
      ({ data: studentRows, error } = await supabase
        .from('students')
  ...
  async getLiveSchoolSubscriptionPlan(schoolId: string): Promise<string | null> {
    ...
    try {
      ...
      // Exclude PENDING (in-flight checkout) rows — only
  ...
  async getStudentMarksheetData(studentId: string, termName: string): Promise<any> {
    ...
    return {
      ...
      student: {
        ...
        // NEVER falls back to users.avatar_url / profile_photo_url (personal profile photos)
        photoUrl: await (async () => {
          try {
            // Step 1: Always query students.registration_photo_url (primary column)
            // This is unconditional — no schema flag gating
            const { data: stRow } = await supabase
              ...
              .eq('school_id', schoolId)
              .maybeSingle();
            if (stRow?.registration_photo_url) {
              console.log(`[Marksheet Photo] Using students.registration_photo_url: ${stRow.registration_photo_url}`);
              return stRow.registration_photo_url;
            }
            ...
            const { data: sp } = await supabase
              ...
              .eq('school_id', schoolId)
              .maybeSingle();
            if (sp?.student_id === studentId && sp?.school_id === schoolId && sp?.photo_url) {
              console.log(`[Marksheet Photo] Using student_profiles.photo_url (fallback): ${sp.photo_url}`);
              return sp.photo_url;
            }
            // Step 3: No academic photo — return empty (shows NO PHOTO placeholder)
            console.log('[Marksheet Photo] No official registration photo found — showing NO PHOTO placeholder');
            return '';
          } catch (err) {
            console.error('[Marksheet Photo] Error fetching photo:', err);
            return '';
          }
        })(),
        // avatarUrl intentionally empty — official documents NEVER use personal profile photo
        avatarUrl: ''
      },
      academic: {
        term: termName,
  ...
  async markSportsAttendance(userId: string, sessionId: string, date: string, attendanceList: any[], device?: string, ip?: string): Promise<any> {
    ...
    if (!attendanceList || attendanceList.length === 0) return { success: true, count: 0 };
  ...
  async fetchSportsAttendanceHistory(schoolId: string, sessionId: string): Promise<any[]> {
    validateSchoolId(schoolId, 'fetchSportsAttendanceHistory');
    const { data, error } = await supabase
      .from('sports_attendance_audit')
      .select('*, students(*, users(first_name, last_name))')
      .eq('school_id', schoolId)
      .eq('session_id', sessionId)
      .order('created_at', { ascending: false });
  ...
  async createSportsTournament(userId: string, tournament: any): Promise<any> {
    const { data, error } = await supabaseAdmin
      .rpc('create_tournament', {
        p_user_id: userId,
        p_name: tournament.name,
        p_sport_id: tournament.sportId,
        p_team_ids: tournament.teamIds || [],
        p_student_ids: tournament.studentIds || [],
        p_opponent: tournament.opponent || '',
        p_venue: tournament.venue,
        p_start_date: tournament.startDate,
        p_end_date: tournament.endDate,
  ...
  async issueSportsCertificate(userId: string, cert: any): Promise<any> {
    const { data, error } = await supabaseAdmin
      .rpc('issue_certificate', {
        p_user_id: userId,
        p_student_id: cert.studentId,
        p_sport_id: cert.sportId,
        p_tournament_id: cert.tournamentId || null,
        p_category: cert.category,
        p_certificate_number: cert.certificateNumber || `AEGIS-SP-${cert.schoolId.substring(0,4)}-${Math.floor(Math.random() * 90000 + 10000)}`,
        p_issue_date: cert.issueDate || new Date().toISOString().split('T')[0],
        p_file_url: cert.fileUrl || '',
        p_verification_qr_code: cert.verificationQrCode || '',
        p_verification_url: cert.verificationUrl || ''
      });
    if (error) throw error;
    return data;
  },

  async uploadCertificatePDF(fileName: string, pdfBlob: Blob): Promise<string> {
    const { data, error } = await supabase.storage
      .from('sports-certificates')
  ...
  async addSportsAchievement(userId: string, ach: any): Promise<any> {
    const { data, error } = await supabaseAdmin
      .rpc('create_achievement', {
        p_user_id: userId,
        p_title: ach.title,
        p_tournament_id: ach.tournamentId || null,
        p_student_id: ach.studentId,
        p_team_id: ach.teamId || null,
        p_sport_id: ach.sportId,
        p_position: ach.position || '',
        p_medal: ach.medal || '',
        p_rank: ach.rank || '',
        p_level: ach.level || 'District',
        p_certificate_eligible: ach.certificateEligible || false,
        p_description: ach.description || null,
        p_images: ach.images || []
      });
    if (error) throw error;
    return data;
  ...
  async upsertSportsMedicalRecord(userId: string, record: any): Promise<any> {
    const { data, error } = await supabaseAdmin
      .rpc('save_medical_fitness', {
        p_user_id: userId,
        p_student_id: record.studentId,
        p_blood_group: record.bloodGroup,
        p_height: record.height ? parseFloat(record.height) : 0,
        p_weight: record.weight ? parseFloat(record.weight) : 0,
        p_bmi: record.bmi ? parseFloat(record.bmi) : 0,
        p_allergies: record.allergies || '',
        p_medical_conditions: record.medicalConditions || '',
        p_emergency_contact: record.emergencyContact || '',
        p_doctor: record.doctor || '',
        p_fitness_expiry_date: record.fitnessExpiryDate,
        p_status: record.status || 'FIT'
      });
    if (error) throw error;
    return data;
  },
