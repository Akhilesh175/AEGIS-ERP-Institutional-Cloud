        ...
        {selectedGroup ? (
          <>
            ...
            <div className="px-6 py-4 bg-slate-900/40 border-b border-slate-800/60 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <button 
                  ...
                  className="p-2 rounded-xl bg-slate-800/60 hover:bg-slate-700/80 border border-slate-700/30 text-slate-300 transition-all flex items-center gap-1.5 text-xs font-semibold"
                >
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="bg-slate-950/70 border border-slate-800/80 rounded-xl pl-8 pr-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-cyan-500/80 transition-colors w-44"
            ...
            {pinnedMessagesList.length > 0 && (
              <div className="px-6 py-2.5 bg-gradient-to-r from-amber-600/10 to-amber-900/5 border-b border-amber-500/20 flex items-center justify-between text-xs text-amber-300/90 font-medium">
            ...
            {activeAnnouncement && (
              <div className="px-6 py-2.5 bg-gradient-to-r from-red-600/10 to-rose-950/5 border-b border-rose-500/25 flex items-center gap-2.5 text-xs text-rose-300">
                <Bell className="h-3.5 w-3.5 text-rose-400 flex-shrink-0 animate-bounce" />
                <span className="font-semibold text-rose-400">Class Announcement:</span>
                  ...
                  if (isSystem) {
                    return (
                      <div key={msg.id} className="flex justify-center my-3">
                        <div className="bg-slate-900/90 border border-slate-800 text-slate-400 px-4 py-1.5 rounded-full text-xs flex items-center gap-2 max-w-lg shadow-sm">
                          <AlertCircle className="h-3.5 w-3.5 text-cyan-400" />
                          <span>{msg.content}</span>
                          <span className="text-[10px] text-slate-600 font-mono ml-1">{dateLabel}</span>
                        </div>
                      </div>
                    );
                  }
                  ...
                  return (
                    <div key={msg.id} className={`flex gap-3 max-w-[85%] ${isCurrentUser ? 'ml-auto flex-row-reverse' : 'mr-auto'}`}>
                      ...
                      <div className="flex flex-col space-y-1 w-full">
                        {/* Meta */}
                        <div className={`flex items-center gap-1.5 text-[11px] ${isCurrentUser ? 'justify-end' : ''}`}>
                          <span className="font-semibold text-slate-300">{msg.senderName}</span>
                          <span className="text-slate-500 font-mono">{dateLabel}</span>
                          {msg.senderRole && (
                        ...
                        {/* Text bubble */}
                        <div className={`p-3.5 rounded-3xl relative group ${
                          isCurrentUser 
                          ...
                          {msg.replyToMessageId && (
                            <div className={`mb-2 p-2 rounded-xl text-[10px] border flex flex-col ${
                              isCurrentUser 
                                ? 'bg-cyan-950/40 border-cyan-800/30 text-cyan-300' 
                                : 'bg-slate-950/60 border-slate-800/50 text-slate-400'
                            }`}>
                              <span className="font-semibold text-[9px] mb-0.5 uppercase tracking-wider">
                          ...
                          {msg.attachments && msg.attachments.length > 0 && (
                            <div className="mt-2.5 space-y-1.5 border-t border-slate-850 pt-2.5">
                              {msg.attachments.map(att => {
                                const isAudio = att.fileType.startsWith('audio') || att.fileName.endsWith('.webm');
                                return (
                                  <div key={att.id} className="flex items-center justify-between p-2 rounded-xl bg-slate-950/30 border border-slate-850 hover:bg-slate-950/50 transition-colors">
                                    <div className="flex items-center gap-2 truncate">
                                      ...
                                      <div className="flex flex-col truncate">
                                        ...
                                        <span className="text-[10px] text-slate-500">{formatBytes(att.fileSize)}</span>
                                      </div>
                                    </div>
                                    {isAudio ? (
                                      <audio src={att.fileUrl} controls className="h-8 max-w-[200px]" />
                                    ) : (
                                      <a
                                        href={att.fileUrl}
                                        target="_blank"
                          ...
                          <div className={`absolute top-2 opacity-0 group-hover:opacity-100 flex gap-1 bg-slate-900 border border-slate-800 rounded-lg p-1 shadow-md transition-opacity z-10 ${
            ...
            {Object.keys(typingUsers).length > 0 && (
              <div className="px-6 py-1.5 text-xs text-slate-500 italic flex items-center gap-1.5 bg-slate-950/20 border-t border-slate-900">
              ...
              <form onSubmit={handleSubmitMessage} className="flex flex-col gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 hover:bg-slate-800 text-slate-400 hover:text-slate-200 transition-colors"
                  >
                  ...
                  <input
                    type="text"
                    placeholder={isRecording ? "Recording voice note..." : "Send a message to class..."}
                    disabled={isRecording || uploadingAttachments}
                    value={messageText}
                    onChange={(e) => setMessageText(e.target.value)}
                    onKeyDown={handleKeyDown}
                    className="flex-1 bg-slate-950/80 border border-slate-800/80 rounded-xl px-4 py-2.5 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-cyan-500/80 transition-colors"
                  />

                  {/* Voice recording controls */}
                  {isRecording ? (
                    <div className="flex items-center gap-2 bg-red-950/20 border border-red-500/30 rounded-xl px-3 py-1.5 text-red-400 text-xs">
                      ...
                      <button 
                        ...
                        className="p-1 rounded bg-slate-800 hover:bg-slate-700 text-emerald-400 ml-1.5"
                      >
                    ...
                    <button
                      ...
                      className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 hover:bg-slate-800 text-slate-400 hover:text-cyan-400 transition-colors"
                    >
                  ...
                  <button
                    type="submit"
                    disabled={isRecording || uploadingAttachments || (!messageText.trim() && attachments.length === 0)}
                    className="p-2.5 rounded-xl bg-gradient-to-r from-cyan-600 to-cyan-500 text-slate-100 hover:from-cyan-500 hover:to-cyan-400 transition-all font-semibold flex items-center justify-center shadow-md disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                ...
                <div className="space-y-2">
                  {sharedFiles.length === 0 ? (
                    ...
                    sharedFiles.map((file) => (
                      <div key={file.id} className="p-2.5 rounded-xl bg-slate-950/30 border border-slate-850 flex items-center justify-between">
                        <div className="flex items-center gap-2 truncate">
                          <FileText className="h-4.5 w-4.5 text-cyan-400 flex-shrink-0" />
                          <div className="flex flex-col truncate">
                            <span className="text-xs truncate font-medium">{file.fileName}</span>
                            <span className="text-[9px] text-slate-500">{formatBytes(file.fileSize)}</span>
                          </div>
                        </div>
                        <a
                          href={file.fileUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="p-1 rounded bg-slate-850 hover:bg-slate-750 text-slate-300"
                        >
                          <Download className="h-3.5 w-3.5" />
                        </a>
                      </div>
              ...
              <div className="space-y-3">
                ...
                {pinnedMessagesList.length === 0 ? (
                  ...
                  pinnedMessagesList.map((m) => (
                    <div key={m.id} className="p-3 bg-slate-950/30 border border-slate-850 rounded-xl relative space-y-2">
